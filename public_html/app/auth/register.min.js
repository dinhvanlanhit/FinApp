...
function alertRegister(message,type) {
...
function f(){
	$("#full_name,#email,#password,#confirm_password").val('');
}
				...
				$.ajax({
					...
					success: function (data) {
						if (data.statusBoolen) {
							$("#alert").html(alertRegister(data.messages,'success'));
							grecaptcha.reset(0);
							f();
							buttonloading('#button-register', false);
						} else {
							$("#alert").html(alertRegister(data.messages,'danger'));
							buttonloading('#button-register', false);
							grecaptcha.reset(0);
						}
					},
					error: function (error) {
						$("#alert").html(alertRegister('<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Emergency notification , The server system is having a problem waiting for a moment!','waring'));
						buttonloading('#button-register', false);
					}
				});
			}else{
                $("#alert").html(alertRegister('<i class="fa fa-exclamation-triangle" aria-hidden="true"></i> Mật khẩu xác nhận không khớp !','danger'));
                buttonloading('#button-register', false);
            }
		}
	});
